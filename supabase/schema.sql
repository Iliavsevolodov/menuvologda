...
-- Production schema. Admin password hash is configured separately and is never committed to GitHub.
...
returns boolean language sql stable security definer set search_path = public, extensions as $$
  select coalesce((select admin_password_hash is not null and extensions.crypt(coalesce(p_password,''), admin_password_hash)=admin_password_hash from public.vote_settings where id=1), false);
$$;
...
returns jsonb language sql stable security definer set search_path = public as $$
  select jsonb_build_object('is_open', is_open and now()<deadline, 'deadline', deadline) from public.vote_settings where id=1;
$$;
...
begin
  ...
  select array_agg(s.value order by s.first_ord) into v_clean from (
    select btrim(value) value,min(ord) first_ord from unnest(coalesce(p_selections,'{}'::text[])) with ordinality as u(value,ord)
    where value is not null and btrim(value)<>'' and char_length(btrim(value))<=120 group by btrim(value)
  ) s;
  ...
  select count(*) into v_invalid from unnest(v_clean) c where not exists(select 1 from public.snack_options o where o.name=c);
...
begin
  ...
  select jsonb_build_object('participants',count(*),'total_selections',coalesce(sum(cardinality(selections)),0),'average_selections',coalesce(avg(cardinality(selections)),0),'is_open',(select is_open and now()<deadline from public.vote_settings where id=1)) into v_result from public.ballots;
  return v_result;
end;
$$;

create or replace function public.admin_results(p_password text)
returns table(snack_name text,votes bigint) language plpgsql stable security definer set search_path = public, extensions as $$
begin
  if not public.check_admin_password(p_password) then raise exception 'ADMIN_ONLY'; end if;
  return query select o.name,coalesce(v.cnt,0)::bigint from public.snack_options o left join (
    select choice,count(*) cnt from public.ballots b cross join lateral unnest(b.selections) choice group by choice
  ) v on v.choice=o.name order by coalesce(v.cnt,0) desc,o.sort_order asc;
end;
$$;
